 ...
 */
public class GetFeedActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final ModelsNotificationResponseForm notification = getIntent().getParcelableExtra("notification");
        ...
        if(notification.getType().equalsIgnoreCase("Post")){
            ...
            Call getNewsPost=mCCService.getSpecificPost(pid,postId);
            getNewsPost.enqueue(feedCallBack);
        }
        else if(notification.getType().equalsIgnoreCase("Event")){
            String postId = notification.getEventId();
            Call getEvent=mCCService.getSpecificEvent(pid,postId);
            getEvent.enqueue(feedCallBack);
        }
    ...
    private void setOnClickListeners(final ModelsFeed feed){
        g_icon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                DBHandler db = new DBHandler(GetFeedActivity.this);
                ...
                if (feed.getStartDate() == null || feed.getStartDate().isEmpty()) {
                    ...
                    if (feed.getIsAttending() != null) {
                        if (feed.getIsAttending().equals("Y")) {
                            ...
                            Toast.makeText(GetFeedActivity.this, "NOT ATTENDING",
                            ...
                            Toast.makeText(GetFeedActivity.this, "ATTENDING",
                                    Toast.LENGTH_SHORT).show();
                        }
...
//
    ...
    private Callback<ModelsCollegeFeed> feedCallBack = new Callback<ModelsCollegeFeed>() {
        @Override
                ...
                if (response.isSuccess()) {
                    ...
                    if (modelsCollegeFeed != null) {
                        ...
                        if (feedItems != null || feedItems.size()>0) {
                            ModelsFeed feed=feedItems.get(0);
                            setUpPage(feed);
                            setOnClickListeners(feed);
                        }
...
//                        mLoadingImageView.setVisibility(View.GONE);
    ...
    private void setUpPage(ModelsFeed feed) {
        ...
        if (feed != null) {
            ...
            try {
                Picasso.with(GetFeedActivity.this).load(feed.getPhotoUrl()).placeholder(R.drawable.default_card_image).into(event_photo);
            } catch (Exception e) {
                e.printStackTrace();
                Picasso.with(GetFeedActivity.this).load(R.drawable.default_card_image).into(event_photo);
            }
            try {
                Picasso.with(GetFeedActivity.this).load(feed.getClubphotoUrl()).into(g_icon);
            } catch (Exception e) {
                e.printStackTrace();
                Picasso.with(GetFeedActivity.this).load(R.mipmap.default_image).into(event_photo);
            }
    ...
    private Callback<ModelsPersonalInfoResponse> getAttendeescallBack = new Callback<ModelsPersonalInfoResponse>() {
        ...
        public void onResponse(Response<ModelsPersonalInfoResponse> response, Retrofit retrofit) {
            ...
            for (int i = 0; i < modelsPersonalResponses.size(); i++) {
                Log.d("pid here too",modelsPersonalResponses.get(i).getPid());}
...
//                bean.setPid(modelsPersonalResponses.get(i).getPid());
